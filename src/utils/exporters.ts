...
export function buildPdfBytes(result: AnalysisResult, context: ExportContext): Uint8Array {
  ...
  doc.addPage()
  doc.setFontSize(14)
  doc.text('Metric Legend', 14, 16)
  doc.setFontSize(9)
  doc.text('Penjelasan simbol dan rumus pada metrik Cyclomatic, Halstead, dan Maintainability.', 14, 22)

  autoTable(doc, {
    startY: 28,
    head: [['Symbol', 'Meaning', 'Formula / Note']],
    body: [
      ['V(G)', 'Cyclomatic Complexity', 'Jumlah jalur independen logika program.'],
      ['E, N, P', 'Edges, Nodes, Connected Components', 'V(G) = E - N + 2P'],
      ['Predicate', 'Titik keputusan logika', 'Contoh: if, else if, for, while, case, operator boolean.'],
      ['n1 / n2', 'Operator unik / Operand unik', 'Jumlah jenis simbol unik yang muncul.'],
      ['N1 / N2', 'Total operator / Total operand', 'Jumlah kemunculan seluruh operator dan operand.'],
      ['Volume', 'Halstead Volume', 'Volume = (N1 + N2) * log2(n1 + n2)'],
      ['Difficulty', 'Halstead Difficulty', 'Difficulty = (n1 / 2) * (N2 / n2)'],
      ['Effort', 'Perkiraan effort implementasi', 'Effort = Difficulty * Volume'],
      ['MI', 'Maintainability Index', 'Skor 0-100, makin tinggi makin mudah dirawat.'],
    ],
  ...
  const footerDate = new Date(context.generatedAt).toLocaleString()
  for (let page = 1; page <= totalPages; page++) {
    doc.setPage(page)
    doc.setFontSize(8)
    doc.setTextColor(100, 116, 139)
    doc.text(
      `Generated by CodeMetric Studio | Built by Jayadev | ${footerDate} | Page ${page}/${totalPages}`,
      14,
      290,
    )
  }
